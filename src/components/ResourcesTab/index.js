...
const ResourcesTab = (props) => {
    ...
    //console.log(newResourceItem,newResourceItem.length)
    useEffect(() => {
        axios.get("https://media-content.ccbp.in/website/react-assignment/resources.json").then(
            res => {setData(res.data);setPerpage(res.data.slice(0,6))}
        )
    },[])
                    ...
                    <div>
                        {searchRender ? (
                            ...
                            <div className='search-keyword-container'>
                                <h1 className='search-head-last'>Please type other Search keyword</h1>
                            </div>
                        )}
